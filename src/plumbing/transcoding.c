        ...
        switch (ts->ts_type) {
            ...
            case SCT_H264:
                ...
                if (strncmp(t->t_props.tp_x264level, "default", 7)==0 || sizeof(t->t_props.tp_x264level)==0) {
                    ...
                    av_dict_set(&opts, "level", t->t_props.tp_x264level, 0);
                    tvhinfo("transcode", "%04X: x264 level set to %s", shortid(t), t->t_props.tp_x264level);
                }
                ...
                if (strncmp(t->t_props.tp_x264tune, "default", 7)==0 || sizeof(t->t_props.tp_x264tune)==0) {
                    ...
                    av_dict_set(&opts, "tune", t->t_props.tp_x264tune, 0);
                ...
                if (t->t_props.tp_vbitrate == 0) {
                    if (t->t_props.tp_quantizermin == 0 && t->t_props.tp_quantizermax == 0) { // all to default
                        ...
                        if (t->t_props.tp_quantizermin > 41) { // cap to 41 so max will be 51 (maximum)
                            t->t_props.tp_quantizermin = 41;
                        }
                        t->t_props.tp_quantizermax = t->t_props.tp_quantizermin + 10;
                    } else
                        if (t->t_props.tp_quantizermin == 0 && t->t_props.tp_quantizermax > 0) { // only max set
                        if (t->t_props.tp_quantizermax < 11) { // cap to 11 so max can be no lower than 1
                            t->t_props.tp_quantizermax = 11;
                        }
                        t->t_props.tp_quantizermin = t->t_props.tp_quantizermax - 10;
                    }
